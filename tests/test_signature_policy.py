...
class SignaturePolicyTests(unittest.TestCase):
    ...
    def _signed_event(self) -> EventEnvelope:
        unsigned_event = EventEnvelope.from_dict(_base_event_dict())
        sig = sign_event(unsigned_event, key_id="dev-key", key=b"super-secret")
        signed_dict = unsigned_event.to_dict()
        signed_dict["signature"] = {"alg": "hmac-sha256", "key_id": "dev-key", "sig": sig}
